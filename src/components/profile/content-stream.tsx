    ...
    const baseItems = [
      {
        ...
        tags: page === 2 ? ['React', 'Hooks', 'localStorage'] : ['React', 'Hooks', 'Performance'],
        engagement: { likes: Math.floor(Math.random() * 50) + 20, comments: Math.floor(Math.random() * 15) + 5, views: Math.floor(Math.random() * 200) + 100 },
        metadata: { language: 'typescript' }
      },
      {
        id: `additional-thought-${page}`,
        type: 'thoughts' as ContentType,
        title: page === 2 ? 'The Future of Frontend Development' : page === 3 ? 'Building Scalable Component Libraries' : 'Remote Work Best Practices',
        content: page === 2 ? 
        ...
        timestamp: new Date(Date.now() - page * 24 * 60 * 60 * 1000 - 12 * 60 * 60 * 1000),
        tags: page === 2 ? ['Frontend', 'React', 'SSR'] : page === 3 ? ['Design Systems', 'Documentation'] : ['Remote Work', 'Productivity'],
        engagement: { likes: Math.floor(Math.random() * 80) + 30, comments: Math.floor(Math.random() * 25) + 10, views: Math.floor(Math.random() * 300) + 150 },
      }
    ]
...
const handleResponse = <T>(response: ApiResponse<T>) => {
  if (response.success) {
    ...
    // Mock project activity
    {
      ...
      excerpt: 'Major update with improved real-time collaboration, enhanced project showcasing, and new developer networking features.',
    ...
    // Mock thoughts
    {
      ...
      content: 'Been thinking about how state management in React has evolved from Redux being the go-to solution to now having so many great options like Zustand, Jotai, and Valtio. Each has its place depending on the use case. What&apos;s your favorite state management solution in 2024?',
  ...
  const renderStreamItem = (item: StreamItem) => {
    ...
    return (
      <motion.div
        ...
        whileHover={{ y: -2 }}
        className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:border-orange-200 hover:shadow-lg transition-all duration-200"
      >
        <div className="p-4">
          {/* Content Header */}
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center space-x-2">
              {item.type === 'snippets' && <Code className="h-5 w-5 text-purple-600" />}
              {item.type === 'projects' && <Rocket className="h-5 w-5 text-green-600" />}
              {item.type === 'thoughts' && <MessageSquare className="h-5 w-5 text-blue-600" />}
              
              <Badge 
                variant="secondary"
                className={`text-xs ${
                  item.type === 'snippets' ? 'bg-purple-100 text-purple-700 border-purple-200' :
                  item.type === 'projects' ? 'bg-green-100 text-green-700 border-green-200' :
          ...
          {/* Content */}
          <div className="mb-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-2 hover:text-orange-600 transition-colors cursor-pointer">
              {item.title}
            </h3>
    ...
    >
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Content Stream</h2>
          ...
          <div className="flex items-center space-x-2">
            <Button
              ...
              className={sortBy === 'recent' ? 'bg-orange-600 hover:bg-orange-700' : ''}
            >
